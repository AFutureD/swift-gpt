...
struct OpenAICompatibleProvider: LLMProvider {
    func generate(
        ...
        let span = startSpan("OpenAICompatibleProvider Generating", context: serviceContext)
        defer { span.end() }
    ...
    func generate(
        ...
        do {
            ...
            let events = try responseBody.map {
                Data($0)
            }.mapToServerSentEvert().prefix {
                $0.data != "[DONE]"
            }.map {
                try decoder.decode(OpenAIChatCompletionStreamResponse.self, from: Data($0.data.utf8))
            }.aggregateToModelStremResponse(with: .init(conversationID: conversation.id))
            
            var iter = events.makeAsyncIterator()
            return AsyncThrowingStream(unfolding: {
                let inner = startSpan("Receive Model Generation", context: span.context)
                defer { inner.end() }
